...
sayHelloButton.addEventListener('mouseover', hoverOn);
...
sayHelloButton.addEventListener('mouseleave', hoverOff)
...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/myron`).then((res) => {
...
function myQuery() {
    axios.get('http://localhost:3000/query-test?name=myron').then((res => {
        console.log(res.data)
    }))
}
...
/*
    ...
    1: Send no queries on the URL -- what happened? Error message: "Error [ERR_HTTP_HEADERS_SENT]"

    2: Send more than 1 query on the URL -- what happened?  Only one value pair, the second value just expands.
*/
...
document.getElementById('food-input').addEventListener('submit', createFood )